...
/// Both quotes floor-then-add-1 the post-trade reserve (>= ceil, so strictly
/// against the trader even on exact division): a buy-then-sell round trip
/// loses at least 1 lamport, so `real_sol_raised` never returns to 0 once
/// anyone has bought. `enable_pump` relies on that when it reads
/// `real_sol_raised == 0 && tokens_sold == 0` as "never traded"; do not
/// switch to neutral rounding without revisiting it.
